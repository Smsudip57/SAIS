...
//     if (!prices || prices.length === 0 || prices.length !== 3) {
...
//       // Check only the relevant condition for current position
//       let expectedValidity;
//       if (index === 1) expectedValidity = "1 Week";
//       else if (index === 2) expectedValidity = "2 Weeks";
//       else if (index === 3) expectedValidity = "1 Month";
//       else {
//         return res.status(400).json({ message: "Too many price entries" });
//       }

//       if (validity !== expectedValidity) {
//         return res.status(400).json({ message: `Invalid validity at position ${index}. Expected "${expectedValidity}", got "${validity}"` });
//       }
...
//         // Use the SAME logic that works in add_product
//         let expectedValidity;
//         if (index === 1) expectedValidity = "1 Week";
//         else if (index === 2) expectedValidity = "2 Weeks";
//         else if (index === 3) expectedValidity = "1 Month";
...
//     if (purchedPrice) updateFields.purchedPrice = purchedPrice;
//     if (expiryDate) {
...
//         updateFields.status = "active";
//         if (!expiryDate) {
//           // Helper function to calculate expiry date based on validity
...
//               default:
//                 return new Date(now + 30 * 24 * 60 * 60 * 1000); // Default to 1 month
//             }
//           };

//           // Find the current service to get its purchased price validity
//           const currentService = await Service.findOne({ serviceId: serviceId });
//           const validity = currentService?.purchedPrice?.validity || "1 Month";
//           updateFields.expiryDate = getExpiryDate(validity);
//         }
//       }
//     }
//     if (productId) {
...
//       }
//       // Use the purchased plan's validity for expiry date
//       const validity = service.purchedPrice?.validity || "1 Month";
//       service.expiryDate = getExpiryDate(validity);
//       await service.save();
//       value.title = approve
...
//         service.terminationDate = null;
//         service.terminationReason = null;
//         // Use the purchased plan's validity for renewal expiry date
//         const validity = service.purchedPrice?.validity || "1 Month";
//         service.expiryDate = getExpiryDate(validity);
//         await service.save();
...
//         // Access the price from the purchedPrice object structure
//         const refundAmount = service.purchedPrice?.price || request.productMongoID.prices[0].price;
